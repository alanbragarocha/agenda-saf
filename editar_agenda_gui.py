...
class EditorAgendaGUI:
    ...
    def criar_menu(self):
        ...
        menu_arquivo.add_command(label="Gerar Word", command=self.gerar_word)
        menu_arquivo.add_separator()
    ...
    def carregar_dados(self):
        ...
        try:
            ...
            messagebox.showerror("Erro", f"Arquivo '{self.arquivo_atual}' não encontrado!")
    ...
    def criar_aba_info_gerais(self):
        ...
        ttk.Entry(frame, textvariable=self.ano_var, width=10).grid(row=0, column=1, sticky=tk.W, pady=5)

        # Nome da Presidente
        ttk.Label(frame, text="Nome da Presidente:").grid(row=1, column=0, sticky=tk.W, pady=5)
        self.presidente_nome_var = tk.StringVar(value=self.dados.get('presidente', {}).get('nome', ''))
        ttk.Entry(frame, textvariable=self.presidente_nome_var, width=50).grid(row=1, column=1, sticky=tk.W, pady=5)

        # Mensagem da Presidente
        ttk.Label(frame, text="Mensagem da Presidente:").grid(row=2, column=0, sticky=tk.NW, pady=5)
        self.presidente_mensagem_text = scrolledtext.ScrolledText(frame, width=60, height=15, wrap=tk.WORD)
        self.presidente_mensagem_text.grid(row=2, column=1, sticky=tk.W, pady=5)
        self.presidente_mensagem_text.insert('1.0', self.dados.get('presidente', {}).get('mensagem', ''))
    ...
    def criar_aba_diretoria(self):
        ...
        ttk.Button(btn_frame, text="Adicionar Membro", command=self.adicionar_membro_diretoria).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Editar Selecionado", command=self.editar_membro_diretoria).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Remover Selecionado", command=self.remover_membro_diretoria).pack(side=tk.LEFT, padx=5)

        # Treeview para lista
        tree_frame = ttk.Frame(main_frame)
        tree_frame.pack(fill=tk.BOTH, expand=True)

        self.tree_diretoria = ttk.Treeview(tree_frame, columns=('Cargo', 'Nome', 'DN', 'Email'), show='headings', height=15)
        ...
        scrollbar_diretoria = ttk.Scrollbar(tree_frame, orient=tk.VERTICAL, command=self.tree_diretoria.yview)
    ...
    def criar_aba_safs(self):
        ...
        ttk.Button(btn_frame, text="Adicionar SAF", command=self.adicionar_saf).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Editar Selecionada", command=self.editar_saf).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Remover Selecionada", command=self.remover_saf).pack(side=tk.LEFT, padx=5)

        tree_frame = ttk.Frame(main_frame)
        tree_frame.pack(fill=tk.BOTH, expand=True)

        self.tree_safs = ttk.Treeview(tree_frame, columns=('Num', 'Nome', 'Presidente', 'Pastor'), show='headings', height=15)
        ...
        scrollbar_safs = ttk.Scrollbar(tree_frame, orient=tk.VERTICAL, command=self.tree_safs.yview)
    ...
    def criar_aba_atividades_planejadas(self):
        ...
        mes_combo = ttk.Combobox(mes_frame, textvariable=self.mes_var, values=meses_nomes, state='readonly', width=20)
        mes_combo.pack(side=tk.LEFT, padx=5)
        mes_combo.bind('<<ComboboxSelected>>', lambda e: self.atualizar_lista_atividades_planejadas())

        btn_frame = ttk.Frame(mes_frame)
        btn_frame.pack(side=tk.LEFT, padx=10)

        ttk.Button(btn_frame, text="Adicionar", command=self.adicionar_atividade_planejada).pack(side=tk.LEFT, padx=2)
        ttk.Button(btn_frame, text="Editar", command=self.editar_atividade_planejada).pack(side=tk.LEFT, padx=2)
        ttk.Button(btn_frame, text="Remover", command=self.remover_atividade_planejada).pack(side=tk.LEFT, padx=2)
        ...
        scrollbar_ativ = ttk.Scrollbar(list_frame, orient=tk.VERTICAL, command=self.listbox_atividades.yview)
    ...
    def criar_aba_atividades_realizadas(self):
        ...
        ttk.Button(btn_frame, text="Adicionar", command=self.adicionar_atividade_realizada).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Editar", command=self.editar_atividade_realizada).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Remover", command=self.remover_atividade_realizada).pack(side=tk.LEFT, padx=5)

        list_frame = ttk.Frame(main_frame)
        list_frame.pack(fill=tk.BOTH, expand=True)

        self.listbox_atividades_realizadas = tk.Listbox(list_frame, height=20)
        scrollbar = ttk.Scrollbar(list_frame, orient=tk.VERTICAL, command=self.listbox_atividades_realizadas.yview)
    ...
    def criar_aba_outras_info(self):
        ...
        miss_frame = ttk.LabelFrame(main_frame, text="Missionário de Oração", padding=10)
        ...
        ttk.Entry(miss_frame, textvariable=self.miss_nome_var, width=40).grid(row=0, column=1, sticky=tk.W, pady=2)

        ttk.Label(miss_frame, text="Data Nasc.:").grid(row=1, column=0, sticky=tk.W, pady=2)
        self.miss_data_var = tk.StringVar()
        ttk.Entry(miss_frame, textvariable=self.miss_data_var, width=20).grid(row=1, column=1, sticky=tk.W, pady=2)

        ttk.Label(miss_frame, text="Campo:").grid(row=2, column=0, sticky=tk.W, pady=2)
        self.miss_campo_var = tk.StringVar()
        ttk.Entry(miss_frame, textvariable=self.miss_campo_var, width=40).grid(row=2, column=1, sticky=tk.W, pady=2)

        ttk.Label(miss_frame, text="WhatsApp:").grid(row=3, column=0, sticky=tk.W, pady=2)
        self.miss_whats_var = tk.StringVar()
        ttk.Entry(miss_frame, textvariable=self.miss_whats_var, width=20).grid(row=3, column=1, sticky=tk.W, pady=2)
        ...
        ttk.Button(obs_btn_frame, text="Adicionar", command=self.adicionar_observacao).pack(side=tk.LEFT, padx=5)
        ttk.Button(obs_btn_frame, text="Editar", command=self.editar_observacao).pack(side=tk.LEFT, padx=5)
        ttk.Button(obs_btn_frame, text="Remover", command=self.remover_observacao).pack(side=tk.LEFT, padx=5)

        self.listbox_observacoes = tk.Listbox(obs_frame, height=10)
        scrollbar_obs = ttk.Scrollbar(obs_frame, orient=tk.VERTICAL, command=self.listbox_observacoes.yview)
    ...
    def atualizar_lista_diretoria(self):
        ...
        for membro in self.dados.get('diretoria', []):
            self.tree_diretoria.insert('', tk.END, values=(
                membro.get('cargo', ''),
                membro.get('nome', ''),
    ...
    def atualizar_lista_safs(self):
        ...
        for saf in self.dados.get('safs', []):
            pres_nome = saf.get('presidente', {}).get('nome', '') if saf.get('presidente') else ''
            pastor_nome = saf.get('pastor', {}).get('nome', '') if saf.get('pastor') else ''
            self.tree_safs.insert('', tk.END, values=(
                saf.get('numero', ''),
                saf.get('nome', ''),
                pres_nome,
                pastor_nome
    ...
    def atualizar_lista_atividades_planejadas(self):
        ...
        meses_map = {'Janeiro': 'janeiro', 'Fevereiro': 'fevereiro', 'Março': 'marco',
        ...
        for ativ in atividades:
            ...
            self.listbox_atividades.insert(tk.END, f"{data_str}{ativ.get('descricao', '')}")
    ...
    def atualizar_lista_atividades_realizadas(self):
        ...
        for ativ in atividades:
            data_str = f"{ativ.get('data', '')} – " if ativ.get('data') else ""
            self.listbox_atividades_realizadas.insert(tk.END, f"{data_str}{ativ.get('descricao', '')}")
    ...
    def dialog_membro_diretoria(self, membro=None):
        ...
        ttk.Label(dialog, text="Cargo:").grid(row=0, column=0, sticky=tk.W, padx=5, pady=5)
        cargo_var = tk.StringVar(value=membro.get('cargo', '') if membro else '')
        ttk.Entry(dialog, textvariable=cargo_var, width=40).grid(row=0, column=1, padx=5, pady=5)

        ttk.Label(dialog, text="Nome:").grid(row=1, column=0, sticky=tk.W, padx=5, pady=5)
        nome_var = tk.StringVar(value=membro.get('nome', '') if membro else '')
        ttk.Entry(dialog, textvariable=nome_var, width=40).grid(row=1, column=1, padx=5, pady=5)

        ttk.Label(dialog, text="Data Nascimento (DD/MM):").grid(row=2, column=0, sticky=tk.W, padx=5, pady=5)
        data_var = tk.StringVar(value=membro.get('data_nascimento', '') if membro else '')
        ttk.Entry(dialog, textvariable=data_var, width=20).grid(row=2, column=1, padx=5, pady=5)

        ttk.Label(dialog, text="Email:").grid(row=3, column=0, sticky=tk.W, padx=5, pady=5)
        ...
        ttk.Label(dialog, text="Endereço:").grid(row=4, column=0, sticky=tk.W, padx=5, pady=5)
        endereco_var = tk.StringVar(value=membro.get('endereco', '') if membro else '')
        ttk.Entry(dialog, textvariable=endereco_var, width=40).grid(row=4, column=1, padx=5, pady=5)
        ...
        ttk.Button(dialog, text="Salvar", command=salvar).grid(row=5, column=1, pady=10)
        ttk.Button(dialog, text="Cancelar", command=dialog.destroy).grid(row=5, column=0, pady=10)
    ...
    def dialog_saf(self, saf=None):
        ...
        ttk.Label(aba_geral, text="Nome da SAF:").grid(row=0, column=0, sticky=tk.W, padx=5, pady=5)
        nome_var = tk.StringVar(value=saf.get('nome', '') if saf else '')
        ttk.Entry(aba_geral, textvariable=nome_var, width=50).grid(row=0, column=1, padx=5, pady=5)

        ttk.Label(aba_geral, text="Endereço da Igreja:").grid(row=1, column=0, sticky=tk.W, padx=5, pady=5)
        endereco_var = tk.StringVar(value=saf.get('endereco', '') if saf else '')
        ttk.Entry(aba_geral, textvariable=endereco_var, width=50).grid(row=1, column=1, padx=5, pady=5)

        # Aba Pastor
        aba_pastor = ttk.Frame(notebook)
        notebook.add(aba_pastor, text="Pastor")

        ttk.Label(aba_pastor, text="Nome:").grid(row=0, column=0, sticky=tk.W, padx=5, pady=5)
        pastor_nome_var = tk.StringVar(value=saf.get('pastor', {}).get('nome', '') if saf and saf.get('pastor') else '')
        ttk.Entry(aba_pastor, textvariable=pastor_nome_var, width=40).grid(row=0, column=1, padx=5, pady=5)

        ttk.Label(aba_pastor, text="Data Nasc. (DD/MM):").grid(row=1, column=0, sticky=tk.W, padx=5, pady=5)
        pastor_data_var = tk.StringVar(value=saf.get('pastor', {}).get('data_nascimento', '') if saf and saf.get('pastor') else '')
        ttk.Entry(aba_pastor, textvariable=pastor_data_var, width=20).grid(row=1, column=1, padx=5, pady=5)

        # Aba Presidente
        aba_pres = ttk.Frame(notebook)
        notebook.add(aba_pres, text="Presidente")

        ttk.Label(aba_pres, text="Nome:").grid(row=0, column=0, sticky=tk.W, padx=5, pady=5)
        pres_nome_var = tk.StringVar(value=saf.get('presidente', {}).get('nome', '') if saf and saf.get('presidente') else '')
        ttk.Entry(aba_pres, textvariable=pres_nome_var, width=40).grid(row=0, column=1, padx=5, pady=5)

        ttk.Label(aba_pres, text="Data Nasc. (DD/MM):").grid(row=1, column=0, sticky=tk.W, padx=5, pady=5)
        pres_data_var = tk.StringVar(value=saf.get('presidente', {}).get('data_nascimento', '') if saf and saf.get('presidente') else '')
        ttk.Entry(aba_pres, textvariable=pres_data_var, width=20).grid(row=1, column=1, padx=5, pady=5)

        ttk.Label(aba_pres, text="Endereço:").grid(row=2, column=0, sticky=tk.W, padx=5, pady=5)
        pres_end_var = tk.StringVar(value=saf.get('presidente', {}).get('endereco', '') if saf and saf.get('presidente') else '')
        ttk.Entry(aba_pres, textvariable=pres_end_var, width=40).grid(row=2, column=1, padx=5, pady=5)

        ttk.Label(aba_pres, text="CEP:").grid(row=3, column=0, sticky=tk.W, padx=5, pady=5)
        pres_cep_var = tk.StringVar(value=saf.get('presidente', {}).get('cep', '') if saf and saf.get('presidente') else '')
        ttk.Entry(aba_pres, textvariable=pres_cep_var, width=20).grid(row=3, column=1, padx=5, pady=5)

        ttk.Label(aba_pres, text="Telefone:").grid(row=4, column=0, sticky=tk.W, padx=5, pady=5)
        pres_tel_var = tk.StringVar(value=saf.get('presidente', {}).get('telefone', '') if saf and saf.get('presidente') else '')
        ttk.Entry(aba_pres, textvariable=pres_tel_var, width=20).grid(row=4, column=1, padx=5, pady=5)

        ttk.Label(aba_pres, text="Email:").grid(row=5, column=0, sticky=tk.W, padx=5, pady=5)
        pres_email_var = tk.StringVar(value=saf.get('presidente', {}).get('email', '') if saf and saf.get('presidente') else '')
        ttk.Entry(aba_pres, textvariable=pres_email_var, width=40).grid(row=5, column=1, padx=5, pady=5)

        # Aba Conselheiro e Aniversário
        aba_outros = ttk.Frame(notebook)
        notebook.add(aba_outros, text="Outros")

        ttk.Label(aba_outros, text="Conselheiro - Nome:").grid(row=0, column=0, sticky=tk.W, padx=5, pady=5)
        cons_nome_var = tk.StringVar(value=saf.get('conselheiro', {}).get('nome', '') if saf and saf.get('conselheiro') else '')
        ttk.Entry(aba_outros, textvariable=cons_nome_var, width=40).grid(row=0, column=1, padx=5, pady=5)

        ttk.Label(aba_outros, text="Conselheiro - Data Nasc. (DD/MM):").grid(row=1, column=0, sticky=tk.W, padx=5, pady=5)
        cons_data_var = tk.StringVar(value=saf.get('conselheiro', {}).get('data_nascimento', '') if saf and saf.get('conselheiro') else '')
        ttk.Entry(aba_outros, textvariable=cons_data_var, width=20).grid(row=1, column=1, padx=5, pady=5)

        ttk.Label(aba_outros, text="Aniversário - Data (DD/MM):").grid(row=2, column=0, sticky=tk.W, padx=5, pady=5)
        aniv_data_var = tk.StringVar(value=saf.get('aniversario', {}).get('data', '') if saf and saf.get('aniversario') else '')
        ttk.Entry(aba_outros, textvariable=aniv_data_var, width=20).grid(row=2, column=1, padx=5, pady=5)

        ttk.Label(aba_outros, text="Aniversário - Anos:").grid(row=3, column=0, sticky=tk.W, padx=5, pady=5)
        aniv_anos_var = tk.StringVar(value=str(saf.get('aniversario', {}).get('anos', '')) if saf and saf.get('aniversario') else '')
        ttk.Entry(aba_outros, textvariable=aniv_anos_var, width=20).grid(row=3, column=1, padx=5, pady=5)

        def salvar():
            nova_saf = {
                "numero": saf.get('numero', len(self.dados['safs']) + 1) if saf else len(self.dados['safs']) + 1,
                "nome": nome_var.get(),
                ...
                "aniversario": {
                    ...
                    "anos": int(aniv_anos_var.get()) if aniv_anos_var.get().isdigit() else 0
                }
            }
    ...
    def dialog_atividade(self, atividade=None, planejada=True):
        ...
        ttk.Label(dialog, text="Data (ex: 15/03 ou '1º Domingo'):").grid(row=0, column=0, sticky=tk.W, padx=5, pady=5)
        data_var = tk.StringVar(value=atividade.get('data', '') if atividade else '')
        ttk.Entry(dialog, textvariable=data_var, width=40).grid(row=0, column=1, padx=5, pady=5)

        ttk.Label(dialog, text="Descrição:").grid(row=1, column=0, sticky=tk.W, padx=5, pady=5)
        desc_var = tk.StringVar(value=atividade.get('descricao', '') if atividade else '')
        ttk.Entry(dialog, textvariable=desc_var, width=40).grid(row=1, column=1, padx=5, pady=5)
        ...
        def salvar():
            ...
            if planejada:
                ...
                if atividade:
                    ...
                    self.dados['atividades_planejadas_2024'][mes_key].append(nova_atividade)
        ...
        ttk.Button(dialog, text="Cancelar", command=dialog.destroy).grid(row=2, column=0, pady=10)
    ...
    def remover_membro_diretoria(self):
        ...
        if messagebox.askyesno("Confirmar", "Deseja realmente remover este membro?"):
            ...
            self.dados['diretoria'] = [m for m in self.dados['diretoria'] if m['cargo'] != cargo]
            self.atualizar_lista_diretoria()
    ...
    def editar_observacao(self):
        ...
        nova_obs = simpledialog.askstring("Editar Observação", "Nova observação:", initialvalue=obs_atual)
        if nova_obs:
    ...
    def remover_observacao(self):
        ...
        if messagebox.askyesno("Confirmar", "Deseja realmente remover esta observação?"):
    ...
    def salvar_dados(self):
        ...
        self.dados['ano'] = int(self.ano_var.get()) if self.ano_var.get().isdigit() else self.dados.get('ano', 2024)
        self.dados['presidente']['nome'] = self.presidente_nome_var.get()
        self.dados['presidente']['mensagem'] = self.presidente_mensagem_text.get('1.0', tk.END).strip()
